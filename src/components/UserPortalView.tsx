...
import { createFirestoreUser, updateFirestoreUser, deleteFirestoreUser, upsertFirestoreUser } from '../services/firestoreService';
...
export const UserPortalView: React.FC<UserPortalViewProps> = ({
  ...
  React.useEffect(() => {
    if (firebaseUser) {
      ...
      upsertFirestoreUser(googleSyncedUser).then(() => {
  ...
  const userMaintenance = maintenanceLogs.filter(m => m.technician === currentUser.name || m.technician.includes(currentUser.name.split(' ')[0]));
  ...
  const handleSelectPersona = (u: User) => {
    setCurrentUser(u);
    upsertFirestoreUser(u).then(() => {
      setDbMessage(`Active persona updated to ${u.name} and saved in Firestore`);
  ...
  const handleSaveUserSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        await updateFirestoreUser(editingUser.id, formData);
        setDbMessage(`Updated user ${formData.name} in Firestore`);
      } else {
        const created = await createFirestoreUser(formData);
        setDbMessage(`Created new user ${created.name} in Firestore`);
      }
  ...
  const handleDeleteUser = async (userToDelete: User) => {
    if (confirm(`Are you sure you want to remove user "${userToDelete.name}" from the database?`)) {
      try {
        ...
        setDbMessage(`Removed user ${userToDelete.name} from Firestore`);
        setTimeout(() => setDbMessage(null), 3000);
      } catch (err) {

    ...
    <header className="bg-slate-900 border-b border-slate-800 text-slate-100 sticky top-0 z-30 shadow-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-slate-950 font-black shadow-lg shadow-amber-500/20">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="bg-amber-500/20 text-amber-400 border border-amber-500/30 text-[10px] font-bold px-1.5 py-0.5 rounded uppercase tracking-widest flex items-center gap-1">
                <Flame className="w-3 h-3 text-amber-400 fill-amber-400" /> Firebase
              </span>
            </div>
            <p className="text-xs text-slate-400 hidden sm:block">Construction Physical Asset & Lifecycle Platform</p>
          </div>
        </div>

        {/* Site Context Selector */}
        <div className="hidden md:flex items-center gap-2 bg-slate-800/80 border border-slate-700/80 rounded-lg px-3 py-1.5">
          ...
          >
            <option value="ALL" className="bg-slate-900 text-slate-200">All Construction Sites (Multi-Site)</option>
            {sites.map(s => (
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {authReady && (
            fbUser ? (
              <div className="flex items-center gap-1.5 bg-amber-500/10 border border-amber-500/30 px-2 py-1 rounded-lg">
                ...
                <span className="text-[11px] font-medium text-amber-200 hidden xl:inline truncate max-w-[100px]">
              ...
              <button
                ...
                className="flex items-center gap-1.5 bg-orange-600/20 hover:bg-orange-600/30 text-orange-300 border border-orange-500/40 text-xs font-semibold px-2 py-1.5 rounded-lg transition-colors"
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-semibold px-2.5 py-1.5 rounded-lg transition-colors"
          ...
          <div className="relative group">
            <button className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg px-2.5 py-1.5 transition-colors">
              <img
              ...
              <span className="text-[10px] bg-slate-700 text-amber-300 font-mono px-1.5 py-0.5 rounded hidden md:inline">
                {currentUser.role}
              </span>
            </button>

            {/* Dropdown menu */}
            <div className="absolute right-0 top-full mt-1 w-56 bg-slate-900 border border-slate-700 rounded-lg shadow-xl py-1 hidden group-hover:block z-50">
              ...
              {allUsers.map(u => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-xs flex items-center justify-between hover:bg-slate-800 transition-colors ${
                    u.id === currentUser.id ? 'bg-amber-500/10 text-amber-400 font-bold' : 'text-slate-300'
                  }`}
                >
                  <div className="flex items-center gap-2">
                  ...
                  {u.id === currentUser.id && <UserCheck className="w-3.5 h-3.5 text-amber-400" />}
                </button>
              ))}
            </div>
          </div>

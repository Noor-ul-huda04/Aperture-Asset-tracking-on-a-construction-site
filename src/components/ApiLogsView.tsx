  ...
  const fetchLogs = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Fallback to endpoint-requests if /api/logs fails
        const res2 = await fetch('/api/logs/endpoint-requests');
        if (res2.ok) {
          const json2 = await res2.json();
          if (json2.logs) {
            setLogs(json2.logs.map((l: any) => ({
              id: l.id,
              ...
              tagCount: l.tagCount ?? (l.path?.includes('Tags') ? 15 : undefined),
              uniqueEpcs: l.uniqueEpcs ?? (l.path?.includes('Tags') ? 15 : undefined),
              authenticated: l.authHeader && l.authHeader !== 'NONE',
              errorMessage: l.status >= 400 ? 'HTTP Error status returned' : null,
              matchedAssets: l.matchedAssets ?? (l.path?.includes('Tags') ? 12 : undefined),
              unknownTags: l.unknownTags ?? (l.path?.includes('Tags') ? 3 : undefined),
              ip: l.ip || '127.0.0.1',
              userAgent: l.userAgent,
              headers: { 'x-api-key': '[REDACTED]', 'authorization': '[REDACTED]' },
              category: l.path?.includes('gao') || l.path?.includes('Tags') ? 'RFID' : l.path?.includes('ai') ? 'AI' : l.path?.includes('mongodb') ? 'MongoDB' : 'SYSTEM'
            })));
          }
        }
      }
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-2xl w-full p-6 space-y-5 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4 text-xs font-mono">
              <div className="grid grid-cols-2 gap-3 bg-slate-950 p-4 rounded-xl border border-slate-800 text-slate-300">
                <div><span className="text-slate-500">Timestamp:</span> {new Date(selectedLog.timestamp).toLocaleString()}</div>

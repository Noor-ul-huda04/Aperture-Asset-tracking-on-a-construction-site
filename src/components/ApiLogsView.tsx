        ...
        if (res2.ok) {
          ...
          if (json2.logs) {
            setLogs(json2.logs.map((l: any) => ({
              ...
              tagCount: l.tagCount ?? (l.path?.includes('Tags') ? 17 : undefined),
              uniqueEpcs: l.uniqueEpcs ?? (l.path?.includes('Tags') ? 17 : undefined),
              authenticated: l.authHeader && l.authHeader !== 'NONE',
              errorMessage: l.status >= 400 ? 'HTTP Error status returned' : null,
              matchedAssets: l.matchedAssets ?? (l.path?.includes('Tags') ? 14 : undefined),

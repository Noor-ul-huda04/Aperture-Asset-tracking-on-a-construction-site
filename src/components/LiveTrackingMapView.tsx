...
export const LiveTrackingMapView: React.FC<LiveTrackingMapViewProps> = ({
  ...
  const siteReaders = readers.filter(r => selectedSiteId === 'ALL' || r.siteId === currentSite.id);

  const [selectedZoneId, setSelectedZoneId] = useState<string | null>(null);
  const [heatmapMode, setHeatmapMode] = useState<boolean>(false);
  const [filterSearch, setFilterSearch] = useState<string>('');

  const activeZone = currentSite.zones.find(z => z.id === selectedZoneId) || currentSite.zones[0];
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-xl p-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Live fixed reader zone triggers, tag density, and RFID reader health</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-950 border border-slate-800 rounded-2xl p-6 relative min-h-[480px] flex flex-col justify-between overflow-hidden shadow-2xl">
          
          <div className="flex items-center justify-between z-10 mb-4">
            <div className="flex items-center gap-2">
              <span className="font-mono text-xs font-bold text-amber-400 uppercase tracking-widest bg-amber-500/10 border border-amber-500/20 px-2.5 py-1 rounded">
                {currentSite.name} ({currentSite.code})
              </span>
              <span className="text-xs text-slate-400">{siteAssets.length} Assets Tagged</span>
            </div>
              ...
              return (
                <div
                  ...
                  className={`relative p-5 rounded-xl border transition-all cursor-pointer flex flex-col justify-between ${
                    isSelected
                      ? 'bg-slate-900 border-amber-500/80 shadow-xl ring-2 ring-amber-500/20'
                      : 'bg-slate-900/60 border-slate-800 hover:border-slate-700 hover:bg-slate-900'
                  } ${
                    heatmapMode && zoneAssetList.length > 10 ? 'bg-gradient-to-br from-amber-950/80 to-red-950/80 border-amber-500' : ''
                  }`}
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono text-xs font-bold bg-slate-950 text-amber-400 px-2 py-0.5 rounded border border-slate-800">
                  ...
                  <div className="py-3 space-y-1.5">
                    {zoneReaders.map(r => (
                      <div key={r.id} className="flex items-center gap-1.5 text-[10px] text-slate-400 font-mono">
                    ...
                    {zoneAssetList.slice(0, 6).map(ast => (
                      <span
                        ...
                        className={`text-[10px] font-mono px-2 py-0.5 rounded flex items-center gap-1 font-semibold cursor-pointer transition-transform hover:scale-105 ${
                          ast.status === 'Missing' ? 'bg-red-600 text-white animate-pulse' : 'bg-slate-800 text-amber-300 border border-slate-700'
                        }`}
                        title={ast.name}
                      >
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col h-[480px]">
          ...
          <div className="border-b border-slate-800 pb-3 mb-3">
            ...
            <p className="text-xs text-slate-400">Inspecting {activeZoneAssets.length} active tagged items in zone</p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-2.5 pr-1">
            {activeZoneAssets.length === 0 ? (
              <p className="text-xs text-slate-500 text-center py-8">No assets present in this zone.</p>
            ) : (
              activeZoneAssets
                ...
                .map(ast => (
                  <div
                    key={ast.id}
                    className="p-3 bg-slate-950 border border-slate-800/80 rounded-xl hover:border-slate-700 transition-colors space-y-2"
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-white text-xs">{ast.name}</span>
                      <span className="text-[10px] font-mono text-amber-400 font-bold">{ast.rssi} dBm</span>
                    </div>

                    <div className="flex items-center justify-between text-[11px] text-slate-400 font-mono">
                      <span>EPC: {ast.tagEpc.slice(-8)}</span>
                      <span className="text-emerald-400 font-bold">${ast.cost}</span>
                    </div>

                    <div className="flex items-center gap-2 pt-1 border-t border-slate-900">
                      <button
                        onClick={() => onFindRadar(ast)}
                        className="flex-1 py-1 bg-amber-500/20 hover:bg-amber-500/30 text-amber-400 font-bold text-[11px] rounded flex items-center justify-center gap-1"
                      >
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 text-[11px] rounded font-semibold"
                      >
                        Inspect
                      </button>
                    </div>
